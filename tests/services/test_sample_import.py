...
def test_sample_import_loads_fixture_and_is_idempotent(connection) -> None:
    first = import_sample_week(connection)
    first_counts = _counts(connection)
    second = import_sample_week(connection)
...
def test_sample_import_rolls_back_invalid_payload(connection) -> None:
    import_sample_week(connection)
    before = _counts(connection)
    invalid = load_sample_payload()
    invalid["weekly_plan"]["items"][0]["planned_minutes"] = 0

    with pytest.raises(ValidationError):
        import_sample_week(connection, invalid)
